...
export const JobApplication = () => {
  ...
  if (!job) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
        <h2 className="text-2xl font-bold mb-2">{t('jobApplication.jobNotFound')}</h2>
        <p className="text-muted-foreground mb-4">{t('jobApplication.jobNotFoundDesc')}</p>
        <Button onClick={() => navigate('/dashboard/jobs')}>{t('jobApplication.backToJobs')}</Button>
      </div>
    );
  }
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type !== 'application/pdf') {
      toast({ title: t('jobApplication.invalidFile'), description: t('jobApplication.uploadPdfOnly'), variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      toast({ title: t('jobApplication.resumeUploaded'), description: t('jobApplication.resumeReady') });
    } catch (error) {
      toast({ title: t('jobApplication.uploadFailed'), description: t('jobApplication.couldNotUpload'), variant: "destructive" });
      setCustomFile(null);
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!token) {
        toast({
          title: t('jobApplication.authError'),
          description: t('jobApplication.pleaseLogin'),
          variant: "destructive"
        });
        navigate('/login');
        return;
      ...
      if (job) {
        ...
        toast({
          title: t('jobApplication.applicationSubmitted'),
          description: t('jobApplication.appliedFor', { title: job.title, company: job.company }),
          variant: "default",
        });
      ...
      toast({
        title: t('jobApplication.applicationFailed'),
        description: error.message || t('jobApplication.somethingWentWrong'),
        variant: "destructive"
      });
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-12">
      <Button
        ...
        onClick={() => navigate(-1)}
      >
        <ArrowLeft className="w-4 h-4" /> {t('jobApplication.backToJobs')}
      </Button>
        ...
        <div className="md:col-span-2 space-y-6">
            ...
            <CardContent className="space-y-4">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-bold text-sm">{t('jobApplication.brainwaveResume')}</div>
                    <div className="text-xs text-muted-foreground">{t('jobApplication.basedOnProfile')}</div>
                  </div>
                </div>
              ...
              >
                <div className="flex items-center justify-between cursor-pointer" onClick={(e) => {
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-bold text-sm">{t('jobApplication.uploadCustomResume')}</div>
                      <div className="text-xs text-muted-foreground">{customFile ? customFile.name : t('jobApplication.uploadPdfMax')}</div>
                    </div>
                  </div>
                ...
                <div className="mt-2" onClick={(e) => e.stopPropagation()}>
                  ...
                  {!customFile && (
                    <Button variant="outline" size="sm" className="w-full mt-2" disabled={isUploading} onClick={() => document.getElementById('resume-upload')?.click()}>
                      ...
                      {isUploading ? t('jobApplication.uploading') : t('jobApplication.clickToUpload')}
                    </Button>
                  )}
                  {customFile && (
                    <div className="flex justify-end mt-2">
                      <Button variant="ghost" size="sm" className="text-red-500 hover:text-red-600 h-8" onClick={() => {
                        ...
                        <Trash2 className="w-4 h-4 mr-2" /> {t('jobApplication.remove')}
                      </Button>
                    </div>
                  )}
                </div>
          ...
          <Card>
            <CardHeader className="py-3">
              <CardTitle className="text-lg flex items-center gap-2">
                <FileText className="w-5 h-5 text-primary" />
                {t('jobApplication.coverLetterOptional')}
              </CardTitle>
            </CardHeader>
        ...
        <div className="space-y-6">
          <Card className="bg-primary/5 border-primary/10 shadow-none">
            <CardContent className="p-6 space-y-4">
              <h3 className="font-bold text-lg">{t('jobApplication.applicationSummary')}</h3>
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{t('jobApplication.jobId')}</span>
                  <span className="font-mono text-xs overflow-hidden text-ellipsis max-w-[100px]">{job.id}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{t('jobApplication.role')}</span>
                  <span className="font-medium text-right">{job.title}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{t('jobApplication.company')}</span>
                  <span className="font-medium">{job.company}</span>
                </div>
                <div className="flex justify-between pt-2 border-t border-border">
                  <span className="text-muted-foreground">{t('jobApplication.resume')}</span>
                  <span className="font-medium">{resumeMode === 'SYSTEM' ? t('jobApplication.systemGenerated') : t('jobApplication.customUpload')}</span>
                </div>
              </div>
              <div className="pt-4 border-t border-primary/10">
                ...
                <p className="text-xs text-muted-foreground text-center mt-3">
                  {t('jobApplication.agreeToShare', { company: job.company })}
                </p>
              </div>
            </CardContent>
          </Card>
